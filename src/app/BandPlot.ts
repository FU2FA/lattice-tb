...
export class BandPlot {
  ...
  clearReference(): void {
    ...
    if (this.lastResult) this.draw(this.lastResult, this.hoverIndex);
  }
  ...
  private plotMetrics() {
    const rect = this.canvas.getBoundingClientRect();
    const margin = { left: 46, right: 16, top: 20, bottom: 36 };
    const plotW = rect.width - margin.left - margin.right;
    const plotH = rect.height - margin.top - margin.bottom;
    return { rect, margin, plotW, plotH };
  }
  ...
  private handleMouseMove(event: MouseEvent): void {
    ...
    const bandX = xMin + ((x - margin.left) / (plotW || 1)) * (xMax - xMin || 1);
    let bestIndex = 0;
    let bestDistance = Number.POSITIVE_INFINITY;
    this.lastResult.points.forEach((point, index) => {
      const d = Math.abs(point.x - bandX);
  ...
  private handleMouseLeave(): void {
    ...
    if (this.lastResult) this.draw(this.lastResult, null);
  }
  ...
  draw(result: BandResult, hoverIndex: number | null = this.hoverIndex): void {
    ...
    if (Math.abs(eMax - eMin) < 1e-8) { eMin -= 1; eMax += 1; }
    const pad = 0.08 * (eMax - eMin);
    eMin -= pad; eMax += pad;

    const xToPx = (x: number, r: BandResult = result) => {
      const xMin = r.points[0]?.x ?? 0;
      const xMax = r.points[r.points.length - 1]?.x ?? 1;
      return margin.left + ((x - xMin) / (xMax - xMin || 1)) * plotW;
    };
    const eToPy = (e: number) => margin.top + (1 - (e - eMin) / (eMax - eMin)) * plotH;
    ...
    const hasWeights = result.colorMode !== undefined && result.colorMode !== "plain";
    ...
    if (result.fermiEnergy !== undefined) {
      ...
      ctx.strokeStyle = "#facc15"; ctx.lineWidth = 1.2; ctx.setLineDash([6, 4]);
      ctx.beginPath(); ctx.moveTo(margin.left, y); ctx.lineTo(margin.left + plotW, y); ctx.stroke(); ctx.setLineDash([]);
      ctx.fillStyle = "#facc15"; ctx.font = "11px sans-serif"; ctx.textAlign = "left"; ctx.textBaseline = "bottom";
    ...
    if (hoverIndex !== null && result.points[hoverIndex]) {
      ...
      ctx.beginPath(); ctx.moveTo(hx, margin.top); ctx.lineTo(hx, margin.top + plotH); ctx.stroke();
      ctx.fillStyle = "#22c55e"; ctx.beginPath(); ctx.arc(hx, margin.top + 8, 4, 0, Math.PI * 2); ctx.fill();
    }

    if (hasWeights) {
      const x0 = width - 145; const y0 = 8;
      const gradient = ctx.createLinearGradient(x0, 0, x0 + 70, 0);
      gradient.addColorStop(0, this.bandColor(0, true)); gradient.addColorStop(1, this.bandColor(1, true));
      ctx.fillStyle = gradient; ctx.fillRect(x0, y0, 70, 8);
      ctx.strokeStyle = "#64748b"; ctx.strokeRect(x0, y0, 70, 8);
      ctx.fillStyle = "#cbd5e1"; ctx.font = "11px sans-serif"; ctx.textAlign = "left"; ctx.textBaseline = "top";
      ctx.fillText(result.colorMode ?? "", x0 + 76, y0 - 2);
    }

    if (this.referenceResult) {
      ctx.strokeStyle = "rgba(148, 163, 184, 0.9)"; ctx.setLineDash([4, 4]);
      ctx.beginPath(); ctx.moveTo(width - 122, height - 18); ctx.lineTo(width - 88, height - 18); ctx.stroke(); ctx.setLineDash([]);
      ctx.fillStyle = "#cbd5e1"; ctx.font = "11px sans-serif"; ctx.textAlign = "left"; ctx.textBaseline = "middle";
